...
Rpush.reflect do |on|
  ...
  on.apns_feedback do |feedback|
    device = Device.includes(:user).find_by(token: feedback.device_token, device_type: 'ios')

    device&.user&.devices&.where(token: device.token, device_type: 'ios')&.destroy_all unless device.nil?
  end
  ...
  on.gcm_canonical_id do |old_id, canonical_id|
    device = Device.includes(:user).find_by(token: old_id, device_type: 'android')

    device&.update(token: canonical_id)
  end
  ...
  on.gcm_invalid_registration_id do |_, _, registration_id|
    device = Device.includes(:user).find_by(token: registration_id, device_type: 'android')

    device&.user&.devices&.where(token: device.token, device_type: 'android')&.destroy_all unless device.nil?
  end
